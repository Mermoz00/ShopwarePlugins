...
{block name="frontend_detail_image"}
    ...
    {block name="frontend_detail_image_box"}
        <div
            class="image-slider--container{if !$sArticle.image} no--image{/if}{if !$sArticle.images|count} no--thumbnails{/if}">
                ...
                {block name='frontend_detail_image_default_image_slider_item'}
                    <div class="image--box image-slider--item">
                        ...
                        {block name='frontend_detail_image_default_image_element'}
                            ...
                            <span class="image--element" {if $sArticle.image} data-img-large="{$sArticle.image.thumbnails[2].source}"
                                    data-img-small="{$sArticle.image.thumbnails[0].source}" data-img-original="{$sArticle.image.source}"
                                {/if} data-alt="{$alt}">
                                    ...
                                    <span class="{$imageMediaClasses}">
                                        {if isset($sArticle.image.thumbnails)}
                                            {block name='frontend_detail_image_default_picture_element'}
                                                <img srcset="{$sArticle.image.thumbnails[1].sourceSet}" src="{$sArticle.image.thumbnails[1].source}"
                                                    alt="{$alt}" itemprop="image" />
                                            {/block}
                    ...
                    {block name='frontend_detail_images_image_slider_item'}
                        <div class="image--box image-slider--item">
                            ...
                            {block name='frontend_detail_images_image_element'}
                                ...
                                <span class="image--element" data-img-large="{$image.thumbnails[2].source}"
                                    data-img-small="{$image.thumbnails[0].source}" data-img-original="{$image.source}" data-alt="{$alt}">
                                        ...
                                        <span class="{$imageMediaClasses}">
                                            {if isset($image.thumbnails)}
                                                {block name='frontend_detail_images_picture_element'}
                                                    ...
                                                    {* div tag because of the jquery.main.js *}
                                                    {* <div id="summary" style="width: 100%"></div> *}
                                                    <model-viewer id="box" src='{$sArticle['three']}'
                                                        style="background-image:url({$threeData['image']});" alt="A 3D model of an astronaut"
